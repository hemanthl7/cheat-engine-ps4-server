...
namespace libdebug
{
	...
	public class PS4DBG
	{
		...
		public List<ulong> ScanProcess<T>(int pid, PS4DBG.ScanCompareType compareType, T value, T extraValue = default(T))
		{
			...
			{
				sbyte value3 = value as sbyte;
				scanValueType = PS4DBG.ScanValueType.valTypeInt8;
				array2 = BitConverter.GetBytes((short)value3);
				num = 1;
				if (extraValue != null)
				{
			...
			else if (value is byte)
			{
				...
				scanValueType = PS4DBG.ScanValueType.valTypeUInt8;
				array2 = BitConverter.GetBytes((short)value4);
				num = 1;
				if (extraValue != null)
				{
			...
			{
				double value12 = value as double;
			...
			for (;;)
			{
				ulong num2 = BitConverter.ToUInt64(this.ReceiveData(8), 0);
